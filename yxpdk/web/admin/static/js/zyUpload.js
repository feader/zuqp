		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					...
					// html += "<input type='hidden' value='"+suid+"' name='suid' />";
			...
			this.createCorePlug = function(){
				var params = {
					...
					onComplete: function(response){
						...
						//console.info(str.upload_img);
						var upload_img_str = $('#upload_img').val();
						if(upload_img_str!=''){
							$('#upload_img').val(upload_img_str+'|'+str.upload_img);
						}else{
							$('#upload_img').val(str.upload_img);
						}
						if(str.code!=3){
							$("#uploadInf").html('');
							$("#uploadInf").append('<p>图片'+str.file_name+','+str.msg+'</p>');	
						}
